<?php
/*
	Plugin Name: Indiewire WP Embeded Player
	Plugin URI: http://wordpress.org/extend/plugins/indiewire-player-embed/
	Description: Adds support for embedding an Indiewire Video on your blog using a shortcode; ex: [indiewire id="ea4af620-a748-11e0-a92a-0026bb61d036"].
	Version: 1.0.0
	Author: SnagFilms
...
*/
if ( !function_exists( 'embed_indiewire_shortcode' ) ) :
	function indiewire_enqueue_script() {
		wp_enqueue_script( 'jquery' );
	}
	add_action('wp_enqueue_scripts', 'indiewire_enqueue_script');
	
	//embed indiewire function
	function embed_indiewire_shortcode($atts, $content = null) {
		$defaults = array(
			'id' => '',
			'width' => '500',
			'height' => '350',
		...
		//Inject Indiewire iframe embed player
		$html = '';
        ...
        $html .= "\n".'<!-- Indiewire Embed Player Plugin v:1.0 - indiewire.com/ -->'."\n";
		$html .= '<iframe';
        foreach ($atts as $attr => $value) {
			if( $attr != 'same_height_as' ){ //remove unused attributes
				if( $value != '' ) { //add attributes to iframe
					if ($attr == 'id') {
						$html .= ' ' . 'src' . '=http://www.indiewire.com/embed/player.jsp?videoId=' . $value . '&w=500';
					} else {
						$html .= ' ' . $attr . '="' . $value . '"';
					}
				}
	...
	add_shortcode('indiewire', 'embed_indiewire_shortcode');
endif;
?>